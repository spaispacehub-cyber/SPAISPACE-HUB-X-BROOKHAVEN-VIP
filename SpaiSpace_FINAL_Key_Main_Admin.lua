...
--====================================================
-- SpaiSpace FINAL MERGED
-- Key System + Main + Admin Panel
-- Created by Huy / 休伊
--====================================================
...
local CONFIG = {
    FreeKey = "Free_Key",
    PremiumPattern = "premium_buyuservip",
    AdminKey = "ADMINVIPBASE"
}

local function getRank(key)
    if key == CONFIG.FreeKey then
        return "FREE"
    elseif string.find(key, CONFIG.PremiumPattern) then
        return "PREMIUM"
    elseif key == CONFIG.AdminKey then
        return "ADMIN"
    end
end

local userKey = getgenv().SpaiSpaceKey or ""
local Rank = getRank(userKey)
...
LocalPlayer:SetAttribute("SpaiSpaceUser", true)
LocalPlayer:SetAttribute("SpaiSpaceRank", Rank)
...
InfoTab:AddParagraph({ "Colaborações:", "GTVZ MODDED " })
InfoTab:AddParagraph({ "Você está usando:", "CHAOS Hub" })
...
---------------- ADMIN PANEL (PREMIUM / ADMIN) ----------------
if Rank == "PREMIUM" or Rank == "ADMIN" then
    local WindUI = loadstring(game:HttpGet("https://raw.githubusercontent.com/Footagesus/WindUI/refs/heads/main/main_example.lua"))()
    local AdminTab = WindUI:CreateTab("SpaiSpace Admin")

    local function isUser(plr)
        return plr:GetAttribute("SpaiSpaceUser") == true
    end

    AdminTab:CreateButton({
        Name="Kill Script Users",
        Callback=function()
            for _,p in pairs(Players:GetPlayers()) do
                if p ~= LocalPlayer and isUser(p) and p.Character then
