...
local Fruits = {
    ...
    {name="Blizzard",   id=44669752, price=2900,  icon="rbxthumb://type=Asset&id=112492519574316&w=150&h=150",  outline=Color3.fromRGB(160,220,255) },
    {name="Lightning",  id=44669756, price=2650,  icon="rbxthumb://type=Asset&id=119528267474923&w=150&h=150",  outline=Color3.fromRGB(255,240,60)  },
    {name="T-Rex",      id=44669760, price=3000,  icon="rbxthumb://type=Asset&id=107790083952677&w=150&h=150",  outline=Color3.fromRGB(80,160,40)   },
    {name="Mammoth",    id=44669764, price=2700,  icon="rbxthumb://type=Asset&id=98540765482910&w=150&h=150",   outline=Color3.fromRGB(200,160,80)  },
    {name="Eagle",      id=44669768, price=2750,  icon="rbxthumb://type=Asset&id=115723875211046&w=150&h=150",  outline=Color3.fromRGB(220,180,40)  },
    -- ── GAMEPASSES ──────────────────────────────────────────────
    {name="2x Mastery",        id=44669772, price=450,  icon="rbxthumb://type=Asset&id=12368273652&w=150&h=150",    outline=Color3.fromRGB(255,200,40)  },
    {name="2x Money",          id=44669775, price=450,  icon="rbxthumb://type=Asset&id=12368273652&w=150&h=150",    outline=Color3.fromRGB(255,200,40)  },
    {name="Premium Ship",      id=44669778, price=800,  icon="rbxthumb://type=Asset&id=12368273652&w=150&h=150",    outline=Color3.fromRGB(100,180,255) },
    {name="Haki Color",        id=44669781, price=300,  icon="rbxthumb://type=Asset&id=12368273652&w=150&h=150",    outline=Color3.fromRGB(80,20,120)   },
    {name="Permanent Tushita", id=44669784, price=2000, icon="rbxthumb://type=Asset&id=12368273652&w=150&h=150",    outline=Color3.fromRGB(240,220,100) },
    {name="Permanent True Triple Katana", id=44669787, price=2000, icon="rbxthumb://type=Asset&id=12368273652&w=150&h=150", outline=Color3.fromRGB(160,140,100) },
    {name="Permanent Yama",    id=44669790, price=2000, icon="rbxthumb://type=Asset&id=12368273652&w=150&h=150",    outline=Color3.fromRGB(80,80,80)    },
    {name="Permanent Pole V2", id=44669793, price=2000, icon="rbxthumb://type=Asset&id=12368273652&w=150&h=150",    outline=Color3.fromRGB(60,180,40)   },
}
...
local function showPurchaseDone(productInfo, recipient)
    ...
    xBtn.TextSize  = 20
    xBtn.TextColor3 = Color3.fromRGB(170, 170, 195)
    xBtn.ZIndex    = 20
    xBtn.Parent    = modal

    -- Checkmark circle — centered horizontally in body
    local CH_SIZE = 72
    ...
    subLbl.Position = UDim2.fromOffset(30, 160)
    ...
    -- Checkmark pops in after modal arrives
...
local function showBuyModal(productInfo, recipient)
    ...
    iconImg.Size        = UDim2.fromOffset(72, 72)
...
Config.BuyDelay = 3
Config.ShowDelay = 2  -- seconds before Buy UI appears

local PANEL_W, PANEL_H = 300, 410
...
titleLbl.Text     = "⚡  SPAISSPACE  •  BLOX FRUITS"
titleLbl.TextXAlignment = Enum.TextXAlignment.Left
titleLbl.ZIndex   = 23
...
panelClose.TextSize           = 20
panelClose.TextColor3         = NEO_CYAN
...
-- ── CARD 3: Show Delay (before Buy UI appears) ──────────────
local card3 = neonCard(280, 116)
neonLabel(card3, "⚡  SHOW DELAY (before Buy UI)", 6, 10, NEO_CYAN)

local showDelayDisplay = Instance.new("TextLabel")
showDelayDisplay.BackgroundTransparency = 1
showDelayDisplay.Size     = UDim2.fromOffset(48, 32)
showDelayDisplay.Position = UDim2.new(0.5, -24, 0, 22)
showDelayDisplay.Font     = Enum.Font.GothamBlack
showDelayDisplay.TextSize = 26
showDelayDisplay.TextColor3 = NEO_CYAN
showDelayDisplay.Text     = tostring(Config.ShowDelay) .. "s"
showDelayDisplay.TextXAlignment = Enum.TextXAlignment.Center
showDelayDisplay.ZIndex   = 23
showDelayDisplay.Parent   = card3

local showMinusBtn = Instance.new("TextButton")
showMinusBtn.Size             = UDim2.fromOffset(34, 34)
showMinusBtn.Position         = UDim2.fromOffset(10, 20)
showMinusBtn.BackgroundColor3 = Color3.fromRGB(14, 14, 28)
showMinusBtn.Text             = "−"
showMinusBtn.Font             = Enum.Font.GothamBold
showMinusBtn.TextSize         = 22
showMinusBtn.TextColor3       = NEO_CYAN
showMinusBtn.AutoButtonColor  = false
showMinusBtn.ZIndex           = 23
showMinusBtn.Parent           = card3
corner(showMinusBtn, 6)
stroke(showMinusBtn, NEO_CYAN, 1, 0.3)

local showPlusBtn = Instance.new("TextButton")
showPlusBtn.Size             = UDim2.fromOffset(34, 34)
showPlusBtn.Position         = UDim2.new(1, -44, 0, 20)
showPlusBtn.BackgroundColor3 = Color3.fromRGB(14, 14, 28)
showPlusBtn.Text             = "+"
showPlusBtn.Font             = Enum.Font.GothamBold
showPlusBtn.TextSize         = 22
showPlusBtn.TextColor3       = NEO_CYAN
showPlusBtn.AutoButtonColor  = false
showPlusBtn.ZIndex           = 23
showPlusBtn.Parent           = card3
corner(showPlusBtn, 6)
stroke(showPlusBtn, NEO_CYAN, 1, 0.3)

showMinusBtn.MouseButton1Click:Connect(function()
    if Config.ShowDelay > 0 then
        Config.ShowDelay = Config.ShowDelay - 1
        showDelayDisplay.Text = tostring(Config.ShowDelay) .. "s"
    end
end)
showPlusBtn.MouseButton1Click:Connect(function()
    if Config.ShowDelay < 30 then
        Config.ShowDelay = Config.ShowDelay + 1
        showDelayDisplay.Text = tostring(Config.ShowDelay) .. "s"
    end
end)

local showPresets = {0, 1, 2, 5}
local showPresetRow = Instance.new("Frame")
showPresetRow.Size             = UDim2.new(1, -16, 0, 24)
showPresetRow.Position         = UDim2.fromOffset(8, 82)
showPresetRow.BackgroundTransparency = 1
showPresetRow.ZIndex           = 23
showPresetRow.Parent           = card3

for i, sec in ipairs(showPresets) do
    local pb = Instance.new("TextButton")
    pb.Size             = UDim2.new(0.23, -3, 1, 0)
    pb.Position         = UDim2.new((i-1)*0.25, 2, 0, 0)
    pb.BackgroundColor3 = Color3.fromRGB(10, 10, 24)
    ...
    pb.Font             = Enum.Font.GothamBold
    pb.TextSize         = 11
    pb.TextColor3       = NEO_CYAN
    pb.AutoButtonColor  = false
    pb.ZIndex           = 24
    pb.Parent           = showPresetRow
    corner(pb, 5)
    stroke(pb, NEO_CYAN, 1, 0.55)
    pb.MouseButton1Click:Connect(function()
        Config.ShowDelay = sec
        showDelayDisplay.Text = sec .. "s"
    end)
end
