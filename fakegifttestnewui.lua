...
local function showPurchaseDone(productInfo, recipient)
    ...
    -- Modal: same wide-short shape as buy modal
    local MW, MH = 600, 240
    ...
    modal.Parent = gui
    corner(modal, 20)
    stroke(modal, Color3.fromRGB(65, 65, 82), 1.5, 0.1)
    ...
    subLbl.Size     = UDim2.fromOffset(MW - 60, 28)
    subLbl.Position = UDim2.fromOffset(30, 122)
    ...
    okBtnBg.Position = UDim2.fromOffset(14, MH - BTN_H - 14)
...
local function showBuyModal(productInfo, recipient)
    ...
    -- ── Modal: wide & short, matches landscape screenshot ──
    -- ~600 wide, ~200 tall
    local MW, MH = 600, 210
    local modal = Instance.new("Frame")
    modal.Size        = UDim2.fromOffset(MW, MH)
    ...
    nameLbl.Size     = UDim2.fromOffset(260, 28)
    ...
    -- Modal: wide & short (landscape style)
    -- W=560, H=280 — button column left, content right
    local MW, MH = 560, 280
    local modal = Instance.new("Frame")
    modal.Size             = UDim2.fromOffset(MW, MH)
    modal.Position         = UDim2.fromScale(0.5, 0.6)
    ...
    modal.Parent           = gui
    corner(modal, 18)
    stroke(modal, Color3.fromRGB(70, 70, 88), 1.5, 0.15)

    -- ============ LEFT: Blue Buy button (full-height column) ============
    local BTN_W = 140
    local buyBtn = Instance.new("TextButton")
    buyBtn.Size             = UDim2.fromOffset(BTN_W, MH)
    buyBtn.Position         = UDim2.fromOffset(0, 0)
    buyBtn.BackgroundColor3 = Config.BuyAccent
    buyBtn.AutoButtonColor  = false
    buyBtn.Text             = ""
    buyBtn.Active           = false
    buyBtn.ZIndex           = 3
    ...
    -- Fix right corners of button to be square (so it merges with modal)
    local btnCapRight = Instance.new("Frame")
    btnCapRight.Size = UDim2.fromOffset(18, MH)
    btnCapRight.Position = UDim2.new(1, -18, 0, 0)
    btnCapRight.BackgroundColor3 = Config.BuyAccent
    btnCapRight.BorderSizePixel = 0
    btnCapRight.ZIndex = 3
    btnCapRight.Parent = buyBtn

    -- Fill bar (dark→blue fill animation)
    local fillBar = Instance.new("Frame")
    fillBar.Size = UDim2.new(1, 0, 0, 0)   -- grows top-down
    fillBar.Position = UDim2.fromScale(0, 0)
    fillBar.AnchorPoint = Vector2.new(0, 0)
    fillBar.BackgroundColor3 = Color3.fromRGB(80, 80, 100)
    fillBar.BorderSizePixel = 0
    fillBar.ZIndex = 2
    fillBar.Parent = buyBtn
    corner(fillBar, 18)
    local fillCapRight = Instance.new("Frame")
    fillCapRight.Size = UDim2.fromOffset(18, MH)
    fillCapRight.Position = UDim2.new(1, -18, 0, 0)
    fillCapRight.BackgroundColor3 = Color3.fromRGB(80, 80, 100)
    fillCapRight.BorderSizePixel = 0
    fillCapRight.ZIndex = 2
    fillCapRight.Parent = fillBar

    -- "Buy" label centered on button
    ...
    -- ============ RIGHT: Content area ============
    local contentX = BTN_W + 18
    local contentW = MW - contentX - 16

    -- Title "Buy item" top-right
    local titleLbl = Instance.new("TextLabel")
    titleLbl.BackgroundTransparency = 1
    titleLbl.Size = UDim2.fromOffset(contentW, 38)
    titleLbl.Position = UDim2.fromOffset(contentX, 14)
    titleLbl.Font = Enum.Font.GothamBold
    ...
    -- Icon (top area, left of content)
    local iconFrame = Instance.new("Frame")
    iconFrame.Size = UDim2.fromOffset(110, 110)
    iconFrame.Position = UDim2.fromOffset(contentX, 56)
    iconFrame.BackgroundColor3 = Color3.fromRGB(18, 18, 24)
    iconFrame.BorderSizePixel = 0
    iconFrame.ZIndex = 3
    iconFrame.Parent = modal
    corner(iconFrame, 12)
    -- fruit-colored outline
    local outlineColor = (productInfo.Outline) or Color3.fromRGB(60, 180, 40)
    stroke(iconFrame, outlineColor, 3, 0)

    local iconImg = Instance.new("ImageLabel")
    iconImg.Size = UDim2.fromOffset(100, 100)
    iconImg.AnchorPoint = Vector2.new(0.5, 0.5)
    iconImg.Position = UDim2.fromScale(0.5, 0.5)
    iconImg.BackgroundTransparency = 1
    iconImg.Image = productInfo.Icon or BLOX_LOGO
    ...
    local nameX = contentX + 120
    local nameLbl = Instance.new("TextLabel")
    nameLbl.BackgroundTransparency = 1
    nameLbl.Size = UDim2.fromOffset(MW - nameX - 16, 30)
    nameLbl.Position = UDim2.fromOffset(nameX, 60)
    nameLbl.Font = Enum.Font.GothamBold
    nameLbl.TextSize = 18
    nameLbl.TextColor3 = Color3.fromRGB(240, 240, 255)
    nameLbl.Text = productInfo.Name
    ...
    -- Price (robux icon + number, below name)
    local priceRow = Instance.new("Frame")
    priceRow.Position = UDim2.fromOffset(nameX, 96)
    priceRow.Size = UDim2.fromOffset(180, 26)
    priceRow.BackgroundTransparency = 1
    priceRow.ZIndex = 4
    priceRow.Parent = modal
    robuxIcon(priceRow, 20, UDim2.fromOffset(0, 3), 4)
    local priceLbl = Instance.new("TextLabel")
    priceLbl.BackgroundTransparency = 1
    priceLbl.Size = UDim2.new(1, -26, 1, 0)
    priceLbl.Position = UDim2.fromOffset(26, 0)
    priceLbl.Font = Enum.Font.GothamBold
    priceLbl.TextSize = 17
    priceLbl.TextColor3 = Color3.fromRGB(240, 240, 255)
    priceLbl.Text = fmtComma(price)
    priceLbl.TextXAlignment = Enum.TextXAlignment.Left
    priceLbl.ZIndex = 4
    ...
    -- Balance display (bottom right, above X)
    local balRow = Instance.new("Frame")
    balRow.Position = UDim2.new(1, -20, 1, -58)
    balRow.AnchorPoint = Vector2.new(1, 0)
    balRow.Size = UDim2.fromOffset(0, 22)
    balRow.AutomaticSize = Enum.AutomaticSize.X
    balRow.BackgroundTransparency = 1
    balRow.ZIndex = 5
    balRow.Parent = modal
    robuxIcon(balRow, 17, UDim2.fromOffset(0, 3), 5)
    local balTxt = Instance.new("TextLabel")
    balTxt.BackgroundTransparency = 1
    balTxt.Position = UDim2.fromOffset(21, 0)
    balTxt.Size = UDim2.new(0, 0, 1, 0)
    ...
    local xBtn = Instance.new("TextButton")
    xBtn.Position = UDim2.new(1, -16, 1, -16)
    ...
    -- Fill bar drains top→bottom over 2.8s, then button activates
    task.spawn(function()
        task.wait(0.5)
        if not fillBar or not fillBar.Parent then return end
        TweenService:Create(fillBar,
