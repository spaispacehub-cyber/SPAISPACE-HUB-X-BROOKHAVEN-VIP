...
local function showPurchaseDone(productInfo, recipient)
    ...
    -- Checkmark image (no circle border)
    local chk = Instance.new("ImageLabel")
    chk.Size             = UDim2.fromOffset(72, 72)
    chk.AnchorPoint      = Vector2.new(0.5, 0)
    chk.Position         = UDim2.new(0.5, 0, 0, 48)
    ...
    -- Pop-in animation
    chk.Size = UDim2.fromOffset(0, 0)
    task.delay(0.25, function()
        if not chk.Parent then return end
        TweenService:Create(chk,
    ...
    subLbl.Position = UDim2.fromOffset(30, 132)
