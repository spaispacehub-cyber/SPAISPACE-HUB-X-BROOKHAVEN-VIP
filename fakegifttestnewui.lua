...
local function showPurchaseDone(productInfo, recipient)
    ...
    local MW, MH = Config.ModalW, Config.ModalH + 40
    local modal = Instance.new("Frame")
    modal.Size        = UDim2.fromOffset(MW, MH)
    modal.Position    = UDim2.fromScale(Config.ModalX, Config.ModalY + 0.1)
    ...
    -- Sub text (no checkmark)
    local subLbl = Instance.new("TextLabel")
    subLbl.BackgroundTransparency = 1
    subLbl.Size     = UDim2.fromOffset(MW - 60, 36)
    subLbl.Position = UDim2.fromOffset(30, 68)
    ...
    -- Slide in from below
    modal:TweenPosition(UDim2.fromScale(Config.ModalX, Config.ModalY),
...
local function showBuyModal(productInfo, recipient)
    ...
    local MW, MH = Config.ModalW, Config.ModalH
    local modal = Instance.new("Frame")
    modal.Size        = UDim2.fromOffset(MW, MH)
    modal.Position    = UDim2.fromScale(Config.ModalX, Config.ModalY + 0.1)
    modal.AnchorPoint = Vector2.new(0.5, 0.5)
...
Config.BuyDelay = 3
-- Modal position/size config (adjustable via settings sliders)
Config.ModalW   = 480
Config.ModalH   = 230
Config.ModalX   = 0.5   -- AnchorPoint scale X (0=left, 0.5=center, 1=right)
Config.ModalY   = 0.5   -- AnchorPoint scale Y
...
-- ── CARD 3: Modal Position & Size sliders ────────────────────
local PANEL_H_NEW = 560
panel.Size = UDim2.fromOffset(PANEL_W, PANEL_H_NEW)
PANEL_H = PANEL_H_NEW

local card3 = neonCard(280, 260)
neonLabel(card3, "⊹  MODAL POSITION & SIZE", 6, 10, NEO_CYAN)

-- Slider helper: creates label + track + thumb, returns {value, onChange}
local function makeSlider(parent, label, yOff, minV, maxV, initV, color)
    local TRACK_W = (PANEL_W - 24 - 16)

    local lbl = Instance.new("TextLabel")
    lbl.BackgroundTransparency = 1
    lbl.Size     = UDim2.new(0.55, 0, 0, 14)
    lbl.Position = UDim2.fromOffset(8, yOff)
    lbl.Font     = Enum.Font.GothamBold
    lbl.TextSize = 10
    lbl.TextColor3 = color or NEO_CYAN
    lbl.Text     = label
    ...
    valLbl.Position = UDim2.new(0.58, 0, 0, yOff)
    valLbl.Font     = Enum.Font.GothamBold
    valLbl.TextSize = 10
    valLbl.TextColor3 = color or NEO_CYAN
    ...
    track.Position         = UDim2.fromOffset(8, yOff + 18)
    track.BackgroundColor3 = Color3.fromRGB(20, 20, 38)
    ...
    local initFrac = (initV - minV) / (maxV - minV)
    fill.Size             = UDim2.new(initFrac, 0, 1, 0)
    fill.BackgroundColor3 = color or NEO_CYAN
    ...
    thumb.Position         = UDim2.new(initFrac, 0, 0.5, 0)
    ...
    local function updateFromX(absX)
        ...
        currentVal = math.floor(minV + frac * (maxV - minV) + 0.5)
    ...
    return {
        ...
        fill = fill, thumb = thumb, valLbl = valLbl,
    }
end

-- W slider: 300–700
local sliderW = makeSlider(card3, "⟷  WIDTH",   22,  300, 700, Config.ModalW, NEO_CYAN)
-- H slider: 160–400
local sliderH = makeSlider(card3, "↕  HEIGHT",  68,  160, 400, Config.ModalH, NEO_CYAN)
-- X slider: 0–100 (percent)
local sliderX = makeSlider(card3, "⊸  POS X %", 114,   0, 100, math.floor(Config.ModalX*100), NEO_PURPLE)
-- Y slider: 0–100 (percent)
local sliderY = makeSlider(card3, "⊸  POS Y %", 160,   0, 100, math.floor(Config.ModalY*100), NEO_PURPLE)

-- Apply button
local applyPos = Instance.new("TextButton")
applyPos.Size             = UDim2.new(1, -16, 0, 26)
applyPos.Position         = UDim2.fromOffset(8, 210)
applyPos.BackgroundColor3 = Color3.fromRGB(0, 40, 100)
applyPos.Text             = "▶  APPLY"
applyPos.Font             = Enum.Font.GothamBlack
applyPos.TextSize         = 11
applyPos.TextColor3       = NEO_CYAN
applyPos.AutoButtonColor  = false
applyPos.ZIndex           = 24
applyPos.Parent           = card3
corner(applyPos, 5)
stroke(applyPos, NEO_CYAN, 1, 0.3)

applyPos.MouseButton1Click:Connect(function()
    Config.ModalW = sliderW.getValue()
    Config.ModalH = sliderH.getValue()
    Config.ModalX = sliderX.getValue() / 100
    Config.ModalY = sliderY.getValue() / 100
    applyPos.Text             = "✔  APPLIED!"
    applyPos.BackgroundColor3 = Color3.fromRGB(0, 100, 50)
    task.delay(1.5, function()
        if applyPos and applyPos.Parent then
            applyPos.Text             = "▶  APPLY"
            applyPos.BackgroundColor3 = Color3.fromRGB(0, 40, 100)
        end
    end)
