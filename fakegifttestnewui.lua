...
local function showPurchaseDone(productInfo, recipient)
    ...
    modal.Parent           = gui
    corner(modal, 16)
    stroke(modal, Color3.fromRGB(60, 60, 75), 1.5, 0.2)

    -- Title
    lbl(modal, {
        Text  = "Giao dịch mua hàng đã hoàn tất",
        Size  = 17,
    ...
    ch.Position    = UDim2.new(0.5, 0, 0, 78)
    ...
    -- Sub-text
    lbl(modal, {
        Text  = "Bạn đã mua " .. productInfo.Name .. ".",
        Size  = 14,
        Color = Config.TextSub,
        Pos   = UDim2.new(0, 20, 0, 152),
    ...
    ok.Position         = UDim2.fromOffset(20, 208)
...
local function showBuyModal(productInfo, recipient)
    ...
    -- Modal dimensions matching reference image: compact, not too tall
    local MODAL_W = 480
    local MODAL_H = 240  -- reduced height to match reference image
    ...
    -- Title "Mua vật phẩm" (Vietnamese, matching reference image)
    lbl(modal, {
        Text  = "Mua vật phẩm",
        Size  = 18,
        Pos   = UDim2.fromOffset(22, 16),
    ...
    -- Item row: centered vertically in the space between divider (y=51) and button (y=180)
    -- Middle zone height = 180 - 51 = 129, row height = 70, so top = 51 + (129-70)/2 = 51+30 = 81
    local ROW_H = 70
    local ROW_Y = 51 + math.floor((129 - ROW_H) / 2)  -- = 81, vertically centered

    local row = Instance.new("Frame")
    row.Position             = UDim2.fromOffset(20, ROW_Y)
    row.Size                 = UDim2.new(1, -40, 0, ROW_H)
    row.BackgroundTransparency = 1
    row.ZIndex               = 3
    row.Parent               = modal

    -- Icon frame: vertically centered in row
    local ICON_SIZE = 60
    local iconFrame = Instance.new("Frame")
    iconFrame.Size             = UDim2.fromOffset(ICON_SIZE, ICON_SIZE)
    iconFrame.Position         = UDim2.new(0, 0, 0.5, -ICON_SIZE/2)  -- vertically centered
    ...
    iconImg.Size                   = UDim2.fromOffset(52, 52)
    ...
    -- Text column: anchored to fill remaining row width, vertically centered
    local textCol = Instance.new("Frame")
    textCol.Position             = UDim2.fromOffset(ICON_SIZE + 16, 0)
    textCol.Size                 = UDim2.new(1, -(ICON_SIZE + 16), 1, 0)
    textCol.BackgroundTransparency = 1
    textCol.ZIndex               = 3
    textCol.Parent               = row

    -- Name label: vertically centered in upper half of text column
    lbl(textCol, {
        Text  = productInfo.Name,
        Size  = 16,
        Pos   = UDim2.new(0, 0, 0, 4),
        FS    = UDim2.new(1, 0, 0, 26),
        Align = Enum.TextXAlignment.Left,
        Z     = 3,
    })

    -- Price row: sits just below name
    ...
    -- Buy button — full width, fixed at bottom, label "Mua"
    local buyBtn = Instance.new("TextButton")
    buyBtn.Position         = UDim2.fromOffset(20, 180)
