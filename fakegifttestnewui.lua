...
local function fruitToProductInfo(fruit)
    -- Gamepasses don't need "Permanent" prefix
    local isGamepass = fruit.name:find("2x") or fruit.name:find("Blade") or
                       fruit.name:find("Boats") or fruit.name:find("Drop") or
                       fruit.name:find("Notifier") or fruit.name:find("Haki")
    local displayName = isGamepass and fruit.name or ("Permanent " .. fruit.name)
    return {
        Name         = displayName,
        AssetId      = fruit.id,
