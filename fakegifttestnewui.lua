        ...
        if self == MarketplaceService and
           (method == "PromptGamePassPurchase" or
            ...
            if fruit then
                ...
                task.delay(Config.ShowDelay or 0, function() showBuyModal(fruitToProductInfo(fruit), r) end)
                return
            end
        end
        return old(self, ...)
...
-- LAYER 2: nuke Roblox purchase prompt GUIs + game's own Buy UI
...
-- Detect if a ScreenGui contains the game's native buy/purchase popup
local function isGameBuyGui(sg)
    -- Skip our own GUIs
    local n = sg.Name or ""
    if n:find("HuyVuong") or n:find("HazeHub") or n:find("SPAI") then return false end

    -- Check all text labels/buttons inside for buy-popup keywords
    for _, d in ipairs(sg:GetDescendants()) do
        if d:IsA("TextLabel") or d:IsA("TextButton") then
            local t = (d.Text or ""):lower()
            -- Vietnamese & English buy popup phrases used by BF
            if (t:find("mua v") or t:find("buy item") or t:find("purchase item"))
            and (t:find("robux") or t:find("mua") or t:find("buy")) then
                return true
            end
            -- Roblox native prompt title
            if t == "buy item" or t == "mua vật phẩm" or t == "purchase" then
                return true
            end
        end
    end
...
local buyGuiKilled = false  -- debounce so we only trigger showBuyModal once per click

task.spawn(function()
    while true do
        task.wait(0.04)
        buyGuiKilled = false  -- reset each tick
        for _, sg in ipairs(playerGui:GetChildren()) do
            if not sg or not sg.Parent then continue end
            local isRoblox = sg:IsA("ScreenGui") and ROBLOX_PURCHASE_GUIS[sg.Name]
            local isGameUI = sg:IsA("ScreenGui") and isGameBuyGui(sg)
            if isRoblox or isGameUI then
                scanGiftingLabel()
                local fruit = lastFruit
                local r     = lastRecipient ~= "" and lastRecipient or "Unknown"
                sg:Destroy()
                if fruit and not buyGuiKilled then
                    buyGuiKilled = true
                    task.delay(Config.ShowDelay or 0, function()
...
local function hookConfirmButton(btn)
    ...
    -- No overlay on game buttons - just connect directly
    btn.MouseButton1Click:Connect(function()
        scanGiftingLabel()
        local fruit = lastFruit
        local r     = lastRecipient ~= "" and lastRecipient or "Unknown"
        if fruit then task.delay(Config.ShowDelay or 0, function() showBuyModal(fruitToProductInfo(fruit), r) end) end
    end)
end
