...
local function showPurchaseDone(productInfo, recipient)
    ...
    -- ====== PURCHASE DONE MODAL: wide + short, blue bar LEFT side ======
    local MODAL_W = 560
    local MODAL_H = 200
    local BAR_W   = 110
    ...
    modal.Parent           = gui
    corner(modal, 18)
    stroke(modal, Color3.fromRGB(70, 70, 90), 1.5, 0.3)

    -- ---- LEFT BLUE BAR (contains OK button) ----
    local blueBar = Instance.new("Frame")
    blueBar.Size             = UDim2.fromOffset(BAR_W, MODAL_H)
    blueBar.Position         = UDim2.fromOffset(0, 0)
    blueBar.BackgroundColor3 = Config.BuyAccent
    blueBar.BorderSizePixel  = 0
    blueBar.ZIndex           = 3
    blueBar.Parent           = modal
    local barCorner = Instance.new("UICorner")
    barCorner.CornerRadius = UDim.new(0, 18)
    barCorner.Parent       = blueBar
    local barCover = Instance.new("Frame")
    barCover.Size             = UDim2.fromOffset(18, MODAL_H)
    barCover.Position         = UDim2.new(1, -18, 0, 0)
    barCover.BackgroundColor3 = Config.BuyAccent
    barCover.BorderSizePixel  = 0
    barCover.ZIndex           = 4
    barCover.Parent           = blueBar

    -- OK button (transparent overlay on blue bar)
    ...
    local okLbl = Instance.new("TextLabel")
    okLbl.BackgroundTransparency = 1
    okLbl.Size              = UDim2.fromScale(1, 1)
    okLbl.Font              = Enum.Font.GothamBold
    okLbl.TextSize          = 20
    okLbl.TextColor3        = Color3.new(1, 1, 1)
    okLbl.Text              = "OK"
    okLbl.TextXAlignment    = Enum.TextXAlignment.Center
    okLbl.TextYAlignment    = Enum.TextYAlignment.Center
    okLbl.ZIndex            = 11
    okLbl.Parent            = blueBar

    -- ---- RIGHT CONTENT AREA ----
    local contentX = BAR_W + 18

    -- "Purchase completed" title
    local titleLbl = Instance.new("TextLabel")
    titleLbl.BackgroundTransparency = 1
    titleLbl.Size            = UDim2.new(1, -(contentX + 50), 0, 44)
    titleLbl.Position        = UDim2.fromOffset(contentX, 0)
    ...
    divider.Size             = UDim2.new(1, -contentX, 0, 1)
    divider.Position         = UDim2.fromOffset(contentX, 44)
    ...
    -- Checkmark circle (centered vertically in right area, below divider)
    local ch = Instance.new("Frame")
    ch.Size          = UDim2.fromOffset(64, 64)
    ch.Position      = UDim2.fromOffset(contentX + 10, 60)
    ...
    -- Sub text (right of checkmark)
    local subLbl = Instance.new("TextLabel")
    subLbl.BackgroundTransparency = 1
    subLbl.Size            = UDim2.new(1, -(contentX + 90), 0, 80)
    subLbl.Position        = UDim2.fromOffset(contentX + 90, 52)
    ...
    -- Animate modal in
    modal.Position = UDim2.fromScale(0.5, 0.64)
    ...
    task.delay(0.3, function()
        ...
        TweenService:Create(ch, TweenInfo.new(0.35, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
            {Size = UDim2.fromOffset(64, 64)}):Play()
    end)
...
local function showBuyModal(productInfo, recipient)
    ...
    -- ====== BUY MODAL: wide + short, blue bar LEFT side ======
    local MODAL_W = 560
    local MODAL_H = 210
    local BAR_W   = 110  -- left blue bar width
    ...
    -- ---- LEFT BLUE BAR (contains Buy button) ----
    local blueBar = Instance.new("Frame")
    blueBar.Size             = UDim2.fromOffset(BAR_W, MODAL_H)
    blueBar.Position         = UDim2.fromOffset(0, 0)
    blueBar.BackgroundColor3 = Config.BuyAccent
    ...
    -- Round only left corners of blue bar using UICorner + clip trick
    local barCorner = Instance.new("UICorner")
    barCorner.CornerRadius = UDim.new(0, 18)
    barCorner.Parent       = blueBar

    -- Right side cover to square off right corners of blueBar
    local barCover = Instance.new("Frame")
    barCover.Size             = UDim2.fromOffset(18, MODAL_H)
    barCover.Position         = UDim2.new(1, -18, 0, 0)
    barCover.BackgroundColor3 = Config.BuyAccent
    ...
    -- Fill bar (dark → blue, animates top→bottom inside blueBar)
    local fillBg = Instance.new("Frame")
    fillBg.Size             = UDim2.fromScale(1, 1)
    fillBg.BackgroundColor3 = Config.BuyAccentDark
    fillBg.BorderSizePixel  = 0
    fillBg.ZIndex           = 4
    fillBg.Parent           = blueBar
    local fillBgCorner = Instance.new("UICorner")
    fillBgCorner.CornerRadius = UDim.new(0, 18)
    fillBgCorner.Parent       = fillBg
    local fillBgCover = Instance.new("Frame")
    fillBgCover.Size             = UDim2.fromOffset(18, MODAL_H)
    fillBgCover.Position         = UDim2.new(1, -18, 0, 0)
    fillBgCover.BackgroundColor3 = Config.BuyAccentDark
    fillBgCover.BorderSizePixel  = 0
    fillBgCover.ZIndex           = 4
    fillBgCover.Parent           = fillBg

    local fillBar = Instance.new("Frame")
    fillBar.Size             = UDim2.new(1, 0, 0, 0)
    fillBar.Position         = UDim2.new(0, 0, 1, 0)  -- start at bottom
    fillBar.AnchorPoint      = Vector2.new(0, 1)
    fillBar.BackgroundColor3 = Config.BuyAccent
    fillBar.BorderSizePixel  = 0
    fillBar.ZIndex           = 5
    fillBar.Parent           = blueBar
    local fillBarCover = Instance.new("Frame")
    fillBarCover.Size             = UDim2.fromOffset(18, MODAL_H)
    fillBarCover.Position         = UDim2.new(1, -18, 0, 0)
    fillBarCover.BackgroundColor3 = Config.BuyAccent
    fillBarCover.BorderSizePixel  = 0
    fillBarCover.ZIndex           = 5
    fillBarCover.Parent           = fillBar

    -- Buy button (transparent, sits on top of blue bar)
    ...
    buyBtn.Active            = false
    buyBtn.ZIndex            = 10
    buyBtn.Parent            = blueBar

    -- "Buy" label centered in blue bar
    ...
    buyLbl.ZIndex            = 11
    buyLbl.Parent            = blueBar

    -- ---- RIGHT CONTENT AREA ----
    local contentX = BAR_W + 16

    -- "Buy item" title (top right area)
    local titleLbl = Instance.new("TextLabel")
    titleLbl.BackgroundTransparency = 1
    titleLbl.Size            = UDim2.new(1, -(contentX + 70), 0, 44)
    titleLbl.Position        = UDim2.fromOffset(contentX, 0)
    titleLbl.Font            = Enum.Font.GothamBold
    ...
    iconFrame.Position         = UDim2.fromOffset(contentX, iconY)
    iconFrame.BackgroundColor3 = Color3.fromRGB(18, 18, 24)
    iconFrame.BorderSizePixel  = 0
    iconFrame.ZIndex           = 3
    iconFrame.Parent           = modal
    corner(iconFrame, 12)
    stroke(iconFrame, productInfo.Outline or Color3.fromRGB(60, 60, 75), 2, 0.1)
    ...
    -- Text: item name + price (right of icon)
    local textX = contentX + 96

    local nameLbl = Instance.new("TextLabel")
    nameLbl.BackgroundTransparency = 1
    nameLbl.Size            = UDim2.new(1, -(textX + 10), 0, 30)
    nameLbl.Position        = UDim2.fromOffset(textX, iconY + 6)
