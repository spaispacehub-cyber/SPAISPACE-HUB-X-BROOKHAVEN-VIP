...
local function showPurchaseDone(productInfo, recipient)
    ...
    -- Wide, low modal  (560 x 220)
    local MODAL_W, MODAL_H = 560, 220
    local SIDE_W = 80   -- blue OK bar width
    ...
    modal.Parent           = gui
    corner(modal, 18)
    stroke(modal, Color3.fromRGB(75, 75, 95), 1.5, 0.2)

    -- ── LEFT: tall blue OK button bar ──
    local okBtn = Instance.new("TextButton")
    okBtn.Size             = UDim2.fromOffset(SIDE_W, MODAL_H)
    okBtn.Position         = UDim2.fromOffset(0, 0)
    okBtn.BackgroundColor3 = Config.BuyAccent
    okBtn.Text             = "OK"
    ...
    -- mask right corners flat
    local okMask = Instance.new("Frame")
    okMask.Size = UDim2.fromOffset(18, MODAL_H)
    okMask.Position = UDim2.new(1, -18, 0, 0)
    okMask.BackgroundColor3 = Config.BuyAccent
    okMask.BorderSizePixel = 0
    okMask.ZIndex = 4
    okMask.Parent = okBtn

    -- hover effect
    okBtn.MouseEnter:Connect(function()
        TweenService:Create(okBtn, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(60,140,255)}):Play()
        TweenService:Create(okMask, TweenInfo.new(0.15), {BackgroundColor3 = Color3.fromRGB(60,140,255)}):Play()
    end)
    okBtn.MouseLeave:Connect(function()
        TweenService:Create(okBtn, TweenInfo.new(0.15), {BackgroundColor3 = Config.BuyAccent}):Play()
        TweenService:Create(okMask, TweenInfo.new(0.15), {BackgroundColor3 = Config.BuyAccent}):Play()
    end)

    -- ── RIGHT CONTENT AREA ──
    local contentX = SIDE_W + 18

    -- "Purchase completed" title (top-right of content area, rotated-style like image)
    local titleLbl = Instance.new("TextLabel")
    titleLbl.BackgroundTransparency = 1
    titleLbl.Size = UDim2.fromOffset(MODAL_W - contentX - 40, 52)
    titleLbl.Position = UDim2.fromOffset(contentX, 10)
    ...
    -- Checkmark circle (center of right area)
    local ch = Instance.new("Frame")
    ch.Size        = UDim2.fromOffset(72, 72)
    ch.Position    = UDim2.fromOffset(contentX + 20, (MODAL_H - 72) / 2)
    ...
    chk.Size = UDim2.fromOffset(38, 38)
    ...
    subLbl.Size = UDim2.fromOffset(MODAL_W - contentX - 120, 56)
    subLbl.Position = UDim2.fromOffset(contentX + 108, (MODAL_H - 56) / 2)
    ...
    -- X close button (bottom-right)
    local xBtn = Instance.new("TextButton")
    xBtn.Position = UDim2.new(1, -14, 1, -14)
    xBtn.AnchorPoint = Vector2.new(1, 1)
    ...
    ch.Position = UDim2.fromOffset(contentX + 56, MODAL_H / 2)
    task.delay(0.3, function()
        ...
        TweenService:Create(ch, TweenInfo.new(0.35, Enum.EasingStyle.Back, Enum.EasingDirection.Out),
            {Size = UDim2.fromOffset(72, 72), Position = chOrigPos}):Play()
    end)
    ...
    okBtn.MouseButton1Click:Connect(finish)
...
local function showBuyModal(productInfo, recipient)
    ...
    -- Wide, low modal (560 x 220)
    local MODAL_W, MODAL_H = 560, 220
    local SIDE_W = 80  -- blue Buy bar width
    ...
    -- ── LEFT: tall blue Buy button bar ──
    local buyBtn = Instance.new("TextButton")
    buyBtn.Size             = UDim2.fromOffset(SIDE_W, MODAL_H)
    ...
    -- mask right corners flat
    local buyMask = Instance.new("Frame")
    buyMask.Size = UDim2.fromOffset(18, MODAL_H)
    buyMask.Position = UDim2.new(1, -18, 0, 0)
    buyMask.BackgroundColor3 = Config.BuyAccentDark
    buyMask.BorderSizePixel = 0
    buyMask.ZIndex = 2
    buyMask.Parent = buyBtn

    -- Fill bar (animates bottom→top, full height)
    ...
    corner(fillBar, 18)
    -- fill mask right side flat
    local fillMask = Instance.new("Frame")
    fillMask.Size = UDim2.fromOffset(18, MODAL_H)
    fillMask.Position = UDim2.new(1, -18, 0, 0)
    fillMask.BackgroundColor3 = Config.BuyAccent
    fillMask.BorderSizePixel = 0
    fillMask.ZIndex = 2
    fillMask.Parent = fillBar
    ...
    -- "Buy item" title (top-right)
    local titleLbl = Instance.new("TextLabel")
    titleLbl.BackgroundTransparency = 1
    titleLbl.Size = UDim2.fromOffset(MODAL_W - contentX - 40, 44)
    titleLbl.Position = UDim2.fromOffset(contentX, 8)
    titleLbl.Font = Enum.Font.GothamBold
    titleLbl.TextSize = 22
    titleLbl.TextColor3 = Color3.fromRGB(240, 240, 255)
    ...
    divider.Size = UDim2.fromOffset(MODAL_W - contentX, 1)
    divider.Position = UDim2.fromOffset(contentX, 52)
    ...
    iconFrame.Size = UDim2.fromOffset(iconSize, iconSize)
    iconFrame.Position = UDim2.fromOffset(contentX, itemY)
    iconFrame.BackgroundColor3 = Color3.fromRGB(18, 18, 24)
    iconFrame.BorderSizePixel = 0
    iconFrame.ZIndex = 3
    iconFrame.Parent = modal
    corner(iconFrame, 12)
    stroke(iconFrame, productInfo.Outline or Color3.fromRGB(60,60,75), 2.5, 0.1)
    ...
    local textX = contentX + iconSize + 14
    local textW = MODAL_W - textX - 16

    -- Item name
    local nameLbl = Instance.new("TextLabel")
    nameLbl.BackgroundTransparency = 1
    nameLbl.Size = UDim2.fromOffset(textW, 30)
    nameLbl.Position = UDim2.fromOffset(textX, itemY + 4)
    ...
    priceRow.Position = UDim2.fromOffset(textX, itemY + 40)
    ...
    -- ── BOTTOM ROW: balance (bottom-right) + X ──
    -- Balance display (bottom-right)
    local balFrame = Instance.new("Frame")
    balFrame.Position = UDim2.new(1, -44, 1, -34)
    ...
    -- Fill animation (bottom → top over 2.8s)
    task.spawn(function()
        task.wait(0.5)
        ...
        TweenService:Create(fillMask,
            TweenInfo.new(2.8, Enum.EasingStyle.Quad, Enum.EasingDirection.In),
            {Size = UDim2.fromOffset(18, MODAL_H)}):Play()
    end)
