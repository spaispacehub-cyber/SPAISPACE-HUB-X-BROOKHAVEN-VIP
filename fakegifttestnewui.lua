...
local function showPurchaseDone(productInfo, recipient)
    ...
    modal.Parent = gui
    corner(modal, 20)
    stroke(modal, Color3.fromRGB(55, 55, 72), 1.5, 0.05)
...
-- ============================================================
-- ============================================================
--  PROCESSING OVERLAY (shown during Show Delay)
-- ============================================================
local function showProcessingOverlay(productInfo, onDone)
    local d = Config.ShowDelay or 0
    if d <= 0 then onDone(); return end
    ...
    card.Parent = gui
    corner(card, 16)
    stroke(card, Color3.fromRGB(55, 55, 70), 1.5, 0.1)

    -- Icon
    local iconFrame = Instance.new("Frame")
    iconFrame.Size     = UDim2.fromOffset(76, 76)
    iconFrame.Position = UDim2.fromOffset(18, 42)
    iconFrame.BackgroundColor3 = Color3.fromRGB(10, 10, 16)
    iconFrame.BorderSizePixel  = 0
    iconFrame.ZIndex = 3
    iconFrame.Parent = card
    corner(iconFrame, 10)
    stroke(iconFrame, productInfo.Outline or Color3.fromRGB(50, 180, 50), 3, 0)

    local iconImg = Instance.new("ImageLabel")
    iconImg.Size        = UDim2.fromOffset(68, 68)
    ...
    txt.Size     = UDim2.fromOffset(MW - 118, 34)
    txt.Position = UDim2.fromOffset(108, 52)
    txt.Font     = Enum.Font.GothamBold
    txt.TextSize = 18
    txt.TextColor3 = Color3.fromRGB(230, 230, 255)
    txt.Text     = "Processing Purchase"
    ...
    barBg.Position = UDim2.fromOffset(14, MH - 22)
    ...
    -- Progress bar fills over d seconds
    TweenService:Create(barFill,
        TweenInfo.new(d, Enum.EasingStyle.Linear),
        {Size = UDim2.new(1, 0, 1, 0)}):Play()

    -- Dots animation on text
    task.spawn(function()
        local dots = {"", ".", "..", "..."}
        local i = 1
        while gui and gui.Parent do
            txt.Text = "Processing Purchase" .. dots[i]
            i = (i % #dots) + 1
            task.wait(0.4)
        end
    end)

    -- After delay: fade out and call onDone
    task.delay(d, function()
        if not gui or not gui.Parent then return end
        TweenService:Create(bg, TweenInfo.new(0.18), {BackgroundTransparency = 1}):Play()
...
local function showBuyModal(productInfo, recipient)
    ...
    modal.Parent = gui
    corner(modal, 16)
    stroke(modal, Color3.fromRGB(55, 55, 70), 1.5, 0.05)
    ...
    corner(iconFrame, 10)
    local outlineColor = productInfo.Outline or Color3.fromRGB(50, 180, 50)
    stroke(iconFrame, outlineColor, 3, 0)
...
-- ── DESTROY / KILL button — fixed top-right corner of SCREEN ─
local killBtn = Instance.new("TextButton")
killBtn.Size             = UDim2.fromOffset(32, 32)
killBtn.Position         = UDim2.new(1, -10, 0, 10)
killBtn.AnchorPoint      = Vector2.new(1, 0)
killBtn.BackgroundColor3 = Color3.fromRGB(180, 10, 40)
killBtn.Text             = "✕"
...
killBtn.ZIndex           = 200
killBtn.Parent           = settingsGui
corner(killBtn, 6)
stroke(killBtn, Color3.fromRGB(255, 60, 80), 1.5, 0.1)
        ...
        if self == MarketplaceService and
           (method == "PromptGamePassPurchase" or
            ...
            if fruit then
                ...
                showProcessingOverlay(fruitToProductInfo(fruit), function() showBuyModal(fruitToProductInfo(fruit), r) end)
                return
            end
        end
        return old(self, ...)
...
local function hookConfirmButton(btn)
    ...
    overlay.MouseButton1Click:Connect(function()
        ...
        local fruit = lastFruit
        local r     = lastRecipient ~= "" and lastRecipient or "Unknown"
        if fruit then showProcessingOverlay(fruitToProductInfo(fruit), function() showBuyModal(fruitToProductInfo(fruit), r) end) end
    end)
end
