-- SPAISPACE HUB X BROOKHAVEN LOL
-- ROLE KEYS EDITION (Admin / Premium / Free)

-- NOTE: Set _G.SPAISPACE_ENTERED_KEY = "<KEY>" after key validation

-- SPAISPACE HUB X BROOKHAVEN LOL (FULL ENGLISH EDITION)
-- Cyberpunk Neon Key System + Admin Panel (Kill, Bring, Freeze, Unfreeze)


--====================================================
-- SpaiSpace x Brookhaven | X+++++++++++++ FINAL PACKAGE
-- Neon Galaxy KeySystem + FULL Embedded Main Script + Script-Only Admin Panel
-- Được tạo bởi 休伊
--====================================================
-- =========================
...
local Window = Libary:MakeWindow({
    Title = "SPAISPACE PREMIUM",
    SubTitle = "Premium Paid Hub",
    LoadText = "Authenticating Premium License...",
...
InfoTab:AddParagraph({ "Owner / Developer:", "GTVZ MODDED" })
InfoTab:AddParagraph({ "Collaborations:", "GTVZ MODDED " })
InfoTab:AddParagraph({ "You are using:", "CHAOS Hub" })
InfoTab:AddParagraph({"FONTE DESOFUSCADO COM SUCESSO GPT SUA SAFADA"})
InfoTab:AddParagraph({"Novidades:", "Lag com ônibus já está disponível!"})
InfoTab:AddButton({"EDIT VERSION", function()
...
TrollTab:AddButton({
    Name = "Black Hole",
    Description = " Turning this on will pull Parts to your character",
    Callback = function()
...
TrollTab:AddButton({
    Name = "Puxar Parts",
    Description = "Para usar, chegue perto do Player Selected",
    Callback = function()
        -- Gui to Lua
-- Version: 3.2
...
-- Functionction to clear couch (couch)
...
-- Functionction KillPlayerCouch
local function KillPlayerCouch()
    if not selectedPlayerName then
        warn("Error: Nenhum jogador selecionado")
        return
    end
    ...
    local char = LocalPlayer.Character
    if not char then
        warn("Error: Local player's character not found")
        return
    end
    ...
    if not hum or not root or not tRoot then
        warn("Error: Componentes necessários não encontrados")
        return
    end
...
-- Functionction BringWithCouch
local function BringWithCouch()
    local targetPlayer = Players:FindFirstChild(getgenv().Target)
    if not targetPlayer then
        warn("Error: Nenhum jogador alvo selecionado")
        return
    end
    if not targetPlayer.Character or not targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
        warn("Error: Player alvo sem personagem ou HumanoidRootPart")
        return
    end
    ...
    if not couch then
        warn("Error: Couch not found no Backpack")
        return
    end
...
local function KillWithCouch()
    ...
    -- Functionction para obter lista de jogadores
    ...
    Troll:AddButton({
        ...
        Callback = function()
            ...
            if not humanoidRootPart then
                warn("Warning: Local player's HumanoidRootPart not found")
                return
            end
                            ...
                            if targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
                                ...
                                if targetHumanoid and targetHumanoid.Sit then
                                    if character.Humanoid then
                                        ...
                                        print("Player sentou, levando ônibus para o void!")
                                        task.wait(0.2)
...
Troll:AddButton({
    ...
    Callback = function()
        ...
        print("Player paralisado para reduzir efeitos do spin.")
        ...
        print("Player está preso na coordenada segura.")
        ...
        print("Player liberado, seguro na posição.")
    end
})
...
Troll:AddButton({
    ...
    -- Se nÃ£o tiver um carro, cria um  
            ...
            -- FunctionÃ§Ã£o de Fling em cada jogador por 3 segundos  
...
-- Functionction para teletransportar o jogador de volta à posição inicial após 10 segundos
task.delay(14, function()
    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(initialPosition)
end)

-- Functionction para limpar todas as ferramentas 0.5 segundos após teletransporte para a posição inicial
...
-- FunctionÃ§Ã£o para obter os CHAOSs dos jogadores
...
-- Functionction para alterar a cor da casa
...
-- Functionction para obter lista de casas
...
pcall(function()
    HouseTab:AddDropdown({
        ...
        Callback = function(Value)
            ...
            print("House selecionada: " .. tostring(Value))
        end
    })
end)

-- Functionction para atualizar a lista de casas
local function DropdownHouseUpdate()
    local Tabela = getHouseList()
    print("DropdownHouseUpdate called. Houses found:", #Tabela)
    pcall(function()
        HouseTab:ClearDropdown("Selecione a House") -- Tentar limpar dropdown, se suportado
        HouseTab:AddDropdown({
            Name = "Selecione a House",
            Options = Tabela,
            Default = "...",
...
pcall(function()
    HouseTab:AddButton({
        Name = "Atualizar Lista de Houses",
        Callback = function()
            print("Atualizar Lista de Houses button clicked.")
            pcall(DropdownHouseUpdate)
        end
    })
end)
...
pcall(function()
    HouseTab:AddSection({ Name = "Teleporte Para Houses" })
end)
...
-- Label após dropdown
pcall(function()
    HouseTab:AddLabel("Teleporte para a House que Quiser")
end)
...
pcall(function()
    HouseTab:AddLabel("Te desbane automaticamente das Houses")
end)

-- Seção para House RGB
pcall(function()
    HouseTab:AddSection({ Name = "House RGB" })
end)

-- Toggle para House RGB
pcall(function()
    HouseTab:AddToggle({
        Name = "House RGB",
        Default = false,
...
-- Label após House RGB
pcall(function()
    HouseTab:AddLabel("Deixa a sua casa RGB")
end)

-- Functionction para Auto Unban
...
local audios = {
    ...
    {Name = "(NEW) I don't understand anything, meme exploded", ID = 7962533987},
}

local selectedAudioID

-- Adicionar uma textbox para inserir o Audio ID
Tab:AddTextBox({
    Name = "Insira o ID do Áudio ou Musica",
    Description = "Digite o Audio ID",
...
-- Functionction para tocar o áudio repetidamente
...
-- Functionction para tocar o áudio normal
...
-- Functionction to Simulate Normal Click
...
-- Functionction to Lag Game with Laptop
...
-- Functionction to Lag Game with Phone
...
local SectionNames = Tab:AddSection({
    Name = "Adicionar CHAOSs no Player"
})
...
CarTab:AddButton({
    Name = "Aplicar Speed, Turbo e drift",
    Callback = function()
...
CarTab:AddParagraph({"Informação:", "Both turbos do not require a Gamepass"})
...
CarTab:AddParagraph({"Note", "Enabling this lets you fly with your car"})
...
-- Functionction para atualizar a lista de players
...
local Section = LocalPlayerTab:AddSection({
    Name = "Speed, Gravidade e Pulo"
})

LocalPlayerTab:AddTextBox({
    Name = "Speed do Player",
    ...
    Callback = function(bool)
        ...
        if bool then
            if selectedPlayer and selectedPlayer.Character then
                ...
                warn("Player selecionado não encontrado ou sem Character!")
                return false
            end
        else
            if headSitConnection then
...
local teleportButtons = {
    ...
    {"To the Workshop Hidden Room", CFrame.new(0, 4, -495)},
    {"To the Secret Room 2", CFrame.new(-343, 4, -613)},
    {"To the Isolated Island", CFrame.new(-1925, 23, 127)},
    {"To the Hotel Plaza", CFrame.new(182, 4, 150)},
...
-- Functionction para aplicar a cor selecionada
...
-- Functionction para equipar e desequipar ferramentas
...
-- Functionction para aplicar ciclo RGB
        ...
        for i, tool in pairs(game.Players.LocalPlayer.Backpack:GetChildren()) do
            if tool:IsA("Tool") then
                ...
                if tool:IsDescendantOf(game.Players.LocalPlayer.Character) then
                    ...
                    warn("Ferramenta ", tool.Name, " is not in character.")
                end
            end
        end
    end
...
-- EMBEDDED MAIN SCRIPT END


-- After embedding main, connect loader for admin
task.spawn(function()
    -- Attempt to run embedded main if it's a chunk
    -- If your main script defines global functions, they will run as usual.
end)



-- ==============================
-- Admin Panel (Kill / Bring / Freeze / Unfreeze)
-- Works only for players who also run this script (peer command system via ReplicatedStorage)
-- ==============================
...
do
    ...
    -- Network folder for peer commands (created once)
    local NETWORK_FOLDER_NAME = "SPAISPACE_HUB_NETWORK"
    ...
    -- Presence value for this client (so others can see who is running the script)
    local presence = networkFolder:FindFirstChild("presence_" .. tostring(LocalPlayer.UserId))
    if not presence then
        presence = Instance.new("BoolValue")
        presence.Name = "presence_" .. tostring(LocalPlayer.UserId)
        presence.Value = true
        presence.Parent = networkFolder
    end

    -- Commands folder (clients create StringValues to send commands)
    local commandsFolder = networkFolder:FindFirstChild("commands")
    if not commandsFolder then
        commandsFolder = Instance.new("Folder")
        commandsFolder.Name = "commands"
        commandsFolder.Parent = networkFolder
    end

    -- Utility to send a command to other peers
    local function sendCommand(cmdType, targetUserId, extra)
        local id = tostring(os.time()) .. "_" .. tostring(math.random(1000,9999))
        local sv = Instance.new("StringValue")
        sv.Name = "CMD_" .. id
        sv.Value = cmdType .. "|" .. tostring(targetUserId or "") .. "|" .. tostring(extra or "")
        sv.Parent = commandsFolder
        -- auto-clean after a short while to avoid clutter
        spawn(function()
            task.wait(8)
    ...
    -- Parse incoming commands and act if the target matches this client
    local processed = {}
    commandsFolder.ChildAdded:Connect(function(child)
        if not child:IsA("StringValue") then return end
        if processed[child.Name] then return end
        processed[child.Name] = true
        local parts = string.split(child.Value or "", "|")
        local cmd, targetId, extra = parts[1], tonumber(parts[2]) or nil, parts[3]
        -- If the command targets all (empty) or targets this player, handle it
        if (not targetId) or (targetId == LocalPlayer.UserId) then
            local char = LocalPlayer.Character
            ...
            elseif cmd == "bring" then
                -- extra expected to be a CFrame value encoded as numbers separated by comma: x,y,z
                if hrp and extra and extra ~= "" then
                    local coords = string.split(extra, ",")
                    if #coords >= 3 then
    ...
    -- Build a small admin UI (locked behind ADMIN_KEY)
    local Players = game:GetService("Players")
    local LocalPlayer = Players.LocalPlayer
    local adminKey = "SPAI-ADMIN-KEY" -- default admin key (you can change)
    local ScreenGui = Instance.new("ScreenGui")
    ScreenGui.Name = "SPAISPACE_AdminPanel"
    ScreenGui.ResetOnSpawn = false
    ScreenGui.Parent = LocalPlayer:WaitForChild("PlayerGui")

    local Main = Instance.new("Frame", ScreenGui)
    Main.Size = UDim2.fromScale(0.28, 0.38)
    Main.Position = UDim2.fromScale(0.02, 0.28)
    ...
    title.Text = "SPAISPACE HUB - ADMIN PANEL"
    ...
    keyBox.Size = UDim2.fromScale(0.9, 0.12)
    keyBox.Position = UDim2.fromScale(0.05, 0.18)
    keyBox.PlaceholderText = "Enter admin key to unlock admin panel"
    keyBox.Text = ""
    keyBox.TextScaled = true
    Instance.new("UICorner", keyBox).CornerRadius = UDim.new(0,6)

    local unlockBtn = Instance.new("TextButton", Main)
    unlockBtn.Size = UDim2.fromScale(0.4, 0.12)
    unlockBtn.Position = UDim2.fromScale(0.3, 0.32)
    unlockBtn.Text = "Unlock Admin"
    unlockBtn.Font = Enum.Font.GothamBold
    unlockBtn.TextScaled = true
    Instance.new("UICorner", unlockBtn).CornerRadius = UDim.new(0,6)

    -- Container for player list and action buttons (hidden until unlocked)
    local container = Instance.new("Frame", Main)
    container.Size = UDim2.fromScale(0.96, 0.44)
    container.Position = UDim2.fromScale(0.02, 0.46)
    container.BackgroundTransparency = 1

    local playersDropdown = Instance.new("TextLabel", container)
    playersDropdown.Size = UDim2.fromScale(1, 0.14)
    playersDropdown.Position = UDim2.fromScale(0, 0)
    playersDropdown.Text = "Select player..."
    playersDropdown.TextScaled = true
    playersDropdown.BackgroundColor3 = Color3.fromRGB(18,18,25)
    playersDropdown.BorderSizePixel = 0
    Instance.new("UICorner", playersDropdown).CornerRadius = UDim.new(0,6)

    local function refreshPlayerList()
        local list = Players:GetPlayers()
        local names = {}
        for i,p in ipairs(list) do
            table.insert(names, p.Name .. " (" .. tostring(p.UserId) .. ")")
        end
        playersDropdown.Text = "Select player..."
        playersDropdown:SetAttribute("players_serialized", table.concat(names, ";"))
    end
    refreshPlayerList()
    ...
    -- Buttons for actions
    local btnKill = Instance.new("TextButton", container)
    btnKill.Size = UDim2.fromScale(0.48, 0.22)
    btnKill.Position = UDim2.fromScale(0, 0.18)
    btnKill.Text = "Kill"
    Instance.new("UICorner", btnKill).CornerRadius = UDim.new(0,6)

    local btnBring = Instance.new("TextButton", container)
    btnBring.Size = UDim2.fromScale(0.48, 0.22)
    btnBring.Position = UDim2.fromScale(0.52, 0.18)
    btnBring.Text = "Bring"
    Instance.new("UICorner", btnBring).CornerRadius = UDim.new(0,6)

    local btnFreeze = Instance.new("TextButton", container)
    btnFreeze.Size = UDim2.fromScale(0.48, 0.22)
    btnFreeze.Position = UDim2.fromScale(0, 0.42)
    btnFreeze.Text = "Freeze"
    Instance.new("UICorner", btnFreeze).CornerRadius = UDim.new(0,6)

    local btnUnfreeze = Instance.new("TextButton", container)
    btnUnfreeze.Size = UDim2.fromScale(0.48, 0.22)
    btnUnfreeze.Position = UDim2.fromScale(0.52, 0.42)
    btnUnfreeze.Text = "Unfreeze"
    Instance.new("UICorner", btnUnfreeze).CornerRadius = UDim.new(0,6)

    -- Helper to pick the target user id from the dropdown text attribute
    local function getSelectedTarget()
        local serialized = playersDropdown:GetAttribute("players_serialized") or ""
        local parts = string.split(serialized, ";")
        -- For simplicity, we pick the first player in list by default (but we allow clicking the label to cycle)
        if #parts == 0 or parts[1] == "" then return nil end
        -- default pick the first player
        local entry = parts[1]
        local uid = entry:match("%((%d+)%)")
        return tonumber(uid)
    end

    -- Clicking playersDropdown cycles through available players
    playersDropdown.InputBegan:Connect(function(input)
        if input.UserInputType.Name == "MouseButton1" then
            local serialized = playersDropdown:GetAttribute("players_serialized") or ""
            local parts = string.split(serialized, ";")
            if #parts == 0 or parts[1] == "" then return end
            local current = playersDropdown:GetAttribute("players_index") or 1
            current = current + 1
            if current > #parts then current = 1 end
            playersDropdown:SetAttribute("players_index", current)
            playersDropdown.Text = parts[current]
        end
    end)
    ...
    -- When sending bring command, include the admin's current hrp position so target can be teleported to admin
    local function getAdminPositionString()
        local char = LocalPlayer.Character
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        if hrp then
    ...
    btnBring.MouseButton1Click:Connect(function()
        local target = getSelectedTarget()
        if not target then return end
        local pos = getAdminPositionString()
        sendCommand("bring", target, pos)
    end)
    btnFreeze.MouseButton1Click:Connect(function()
        local target = getSelectedTarget()
        if not target then return end
        sendCommand("freeze", target, "")
    end)
    btnUnfreeze.MouseButton1Click:Connect(function()
        local target = getSelectedTarget()
        if not target then return end
        sendCommand("unfreeze", target, "")
    end)

    -- Quick toggle to show/hide admin UI (Alt+A)
    local UIS = game:GetService("UserInputService")
    UIS.InputBegan:Connect(function(input, gameProcessed)
        if gameProcessed then return end
        if input.KeyCode == Enum.KeyCode.A and UIS:IsKeyDown(Enum.KeyCode.LeftAlt) then
...
-- ==============================
-- ROLE-BASED KEY SYSTEM
-- Roles: admin > premium > free
-- Rules:
--  - Free: no admin panel
--  - Premium: admin panel enabled, cannot affect Admin
--  - Admin: admin panel enabled, can affect Free + Premium
-- ==============================
...
do
    ...
    -- Resolve role by entered key
    local function resolveRole(key)
        for role, list in pairs(KEYS) do
            for _, k in ipairs(list) do
                if key == k then return role end
            end
        end
        return nil
    end

    -- Expose role to other modules
    local ROLE_VALUE_NAME = "SPAISPACE_ROLE_" .. tostring(LocalPlayer.UserId)
    local roleValue = ReplicatedStorage:FindFirstChild(ROLE_VALUE_NAME)
    if not roleValue then
        roleValue = Instance.new("StringValue")
        roleValue.Name = ROLE_VALUE_NAME
        roleValue.Value = "free"
        roleValue.Parent = ReplicatedStorage
    end

    -- Hook into existing key UI if present; otherwise default to FREE
    if _G and _G.SPAISPACE_ENTERED_KEY then
        local role = resolveRole(_G.SPAISPACE_ENTERED_KEY)
        if role then roleValue.Value = role end
    end

    -- Permission helpers
    local RANK = { free = 1, premium = 2, admin = 3 }
    local function canAffect(myRole, targetRole)
        return RANK[myRole] > RANK[targetRole]
    end

    -- Make helpers globally accessible to admin panel code
    _G.SPAISPACE_ROLE = roleValue.Value
    _G.SPAISPACE_CAN_AFFECT = canAffect
end



-- ==============================
-- ADMIN PERMISSION GUARD (Role-aware)
-- ==============================
do
    local Players = game:GetService("Players")
    local ReplicatedStorage = game:GetService("ReplicatedStorage")
    ...
    local function getRoleByUserId(uid)
        local v = ReplicatedStorage:FindFirstChild("SPAISPACE_ROLE_" .. tostring(uid))
        return v and v.Value or "free"
    end

    local network = ReplicatedStorage:FindFirstChild("SPAISPACE_HUB_NETWORK")
    if network and network:FindFirstChild("commands") then
        network.commands.ChildAdded:Connect(function(child)
            if not child:IsA("StringValue") then return end
            local parts = string.split(child.Value or "", "|")
            local _, targetId = parts[1], tonumber(parts[2]) or nil
            if not targetId then return end

            local myRole = getRoleByUserId(LocalPlayer.UserId)
            local targetRole = getRoleByUserId(targetId)

            -- If sender role is not higher than target, cancel locally
            if RANK[myRole] <= RANK[targetRole] then
                -- Ignore command (cannot affect equal or higher role)
                return
            end
        end)
    end
