print("✅ Security verification passed. Loading SpaceDev...")
...
local Update = (loadstring(Game:HttpGet("https://raw.githubusercontent.com/spaispacehub-cyber/SPAISPACE-HUB-X-BROOKHAVEN-VIP/refs/heads/main/SPAISPACEHUB%20X%20RTaO_UI_1.lua")))();
...
local MainTab = Library:Tab("Main", "rbxassetid://136979691479989");
...
_G.Settings = {
	...
	Combat = {
		...
		["Aimbot Gun (soon update)"] = false,
		["Aimbot Skill Neares (soon update)"] = false,
		["Aimbot Skill (soon update)"] = false,
		["Enable PvP"] = false
	},
	Raid = {
		["Selected Chip"] = nil,
...
(getgenv()).Load = function()
	if readfile and writefile and isfile and isfolder then
		if not isfolder("SPAISPACEHUB") then
			makefolder("SPAISPACEHUB");
		end;
		if not isfolder("SPAISPACEHUB/Blox Fruits/") then
			makefolder("SPAISPACEHUB/Blox Fruits/");
		end;
		if not isfile(("SPAISPACEHUB/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json")) then
			writefile("SPAISPACEHUB/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(_G.Settings));
		else
			local Decode = (game:GetService("HttpService")):JSONDecode(readfile("SPAISPACEHUB/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json"));
	...
	if Update:SaveSettings() then
		if readfile and writefile and isfile and isfolder then
			if not isfile(("SPAISPACEHUB/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json")) then
				...
				writefile("SPAISPACEHUB/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(Array));
			end;
		else
			return warn("Status : Undetected Executor");
		end;
